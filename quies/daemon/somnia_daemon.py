...
@app.route("/debug/cli-test", methods=["POST"])
def debug_cli_test():
    """Run a minimal CLI test and return raw output for debugging."""
    auth_type, token = get_claude_auth()
    if not token:
        return jsonify({"error": "No auth"}), 500

    test_prompt = '''Reply with ONLY this JSON block, no other text:

```json
{
  "summary": "CLI test successful",
  "operations": []
}
```'''
    ...
    # Test 1: with --print (current production path)
    r1 = subprocess.run(
    ...
    try:
        ...
        p2 = {"_raw": r2.stdout[:2000]}

    # Test 3: --print only, no --output-format json (raw text output)
    r3 = subprocess.run(
    ...
    return jsonify({
        "with_print_json": {
            ...
            "result": p1.get("result", "__MISSING__"),
            "output_tokens": p1.get("usage", {}).get("output_tokens"),
            "keys": list(p1.keys()),
        },
        "without_print_json": {
            "exit_code": r2.returncode,
            "result": p2.get("result", "__MISSING__"),
            "output_tokens": p2.get("usage", {}).get("output_tokens"),
            "keys": list(p2.keys()),
        },
        "print_only_text": {
